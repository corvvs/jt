...
import { resolvedPinsAtom, ResolvedPin } from '@/states/pins';
 ...
 * pending index を介して effect で行う (useMatchNavigation と違い
...
export const usePinNavigation = (itemViewRef: React.MutableRefObject<any>) => {
  ...
  const jumpToPin = (resolved: ResolvedPin) => {
    const { item } = resolved;
    if (!item) { return; }
  ...
  useEffect(() => {
    if (pendingIndex === null || !visibles) { return; }
    setPendingIndex(null);
    const visibleIndex = visibles.visibleItems.findIndex((it) => it.index === pendingIndex);
    if (visibleIndex < 0) {
  ...
  }, [pendingIndex, visibles, itemViewRef]);
